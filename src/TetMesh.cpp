...
namespace TetEmbeddedDeform {
    ...
    void TetMesh::setPoints(std::vector<Vector3> &&points) {
        _points = std::move(points);
    }

    void TetMesh::setVerts(std::vector<std::array<size_t, 4>> &&verts) {
    ...
    size_t TetMesh::getNumPoints() const {
        return _points.size();
    }

    size_t TetMesh::getNumCells() const {
        return _cellVerts.size();
    }

    const std::vector<Vector3> &TetMesh::getPoints() const {
        return _points;
    }

    const Vector3 &TetMesh::getPoint(size_t index) const {
        return _points[index];
    }

    const std::array<size_t, 4> &TetMesh::getCellVerts(size_t cell) const {
        return _cellVerts[cell];
    }

    const std::vector<size_t> &TetMesh::getVertCells(size_t vert) const {
        return _vertCells[vert];
    }

    Vector3 TetMesh::getCentroid(size_t cell) const {
        return _centroids[cell];
    }

    Matrix3 TetMesh::getEdgeBasis(size_t cell) const {
        return _edgeBases[cell];
    }

    Matrix3 TetMesh::getEdgeBasisInv(size_t cell) const {
        return _edgeBasesInv[cell];
    }

    std::vector<Embedding> TetMesh::computeEmbedding(const std::vector<Vector3> &points) const {
        Matrix3X pointsMat;
        ...
        for (size_t i = 0; i < points.size(); ++i) {
            ...
            // Check if no enclosing cell was found.
            if (!embeddings[i].isInside()) {
    ...
    void TetMesh::computeEdgeBases(bool computeInv) {
        const size_t ncells = _cellVerts.size();
    ...
    void TetMesh::computeBarycentricCoords(const Vector3 &point, Embedding *e) const {
